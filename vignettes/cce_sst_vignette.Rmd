...
#### data for 1916 and 2015 are incompelte
##### keep this in mind 
...
### try looking at data by season
### we need to add season variable
...
seasonal_temp %>%
  ...
  facet_wrap(~season)
```
###like what we did before, sea highest & lowest of each season
```{r}
seasonal_temp %>%
  #filter(year!=1916 & year!=2015) %>%
  ggplot(aes(x = year)) +
...
```{r}
seasonal_temp %>%
  ggplot(aes(x = year, y = diff, color=season)) +
  geom_path() +
  labs(x = "Year",
       y = "Sea-Surface Temperature (C)",
       title = "Difference") +
