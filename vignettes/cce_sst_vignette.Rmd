...
tmp %>% group_by(year) %>% count() %>% head(14)
```

we see that initial years contain large amount of invalid/missing data \
data for 1916 and 2015 are incomplete
keep this in mind \
consider looking data from 1930 onward

#### just curious, what has been the lowest and highest temperatrues
```{r}
...
#### Calculate annual average , lowest and highest temperature over years
```{r}
...
#### consider looking at data by season
we can create a new variable season using dates \
so when does f/w/s/u start?
#### actually different each year (https://www.nationalgeographic.org/encyclopedia/season/)
```{r}
watertemp <- watertemp %>%
    mutate(month_day=as.Date(paste(2224, month, day, sep = "/" )))
```
##### for our purpose, we will let each 
...
#### now, calc seasonal avergaes across years
```{r}
watertemp %>%
  group_by(season) %>%
...
seasonal_temp %>%
  ...
  facet_wrap(~season)
```
winter below average, expecting that
we see that spring is also below average, possibly a result of water having a higher heat capacity(meaning it takes more energy to increase the temperature of water compared to other substances)
...
seasonal_temp %>%
  ...
  facet_wrap(~season)
```
compared to other seasons, when ocean enters winter, seasurfacetemperature does not vary that much, 
